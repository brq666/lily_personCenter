...
public class ResourceServiceImpl implements ResourceService {
    ...
    @Override
    public List<SysResource> getMenuResources(Long userId) {
        List<SysResource> resources = sysResourceMapper.getMenuResources(userId);
    ...
    private void validateResource(SysResource resource) {
        ...
        if (Const.RESOURCE_MENU.equals(resource.getResourceType()) && resource.getTheOrder() == null) {
            throw new SystemException(ErrorCode.MODIFY_DATA_FAILED, "菜单资源排序不能为空！");
        }

        if (Const.RESOURCE_MENU.equals(resource.getResourceType())) {
